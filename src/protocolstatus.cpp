...
uint32_t ProtocolStatus::playerAmount = 0;
uint32_t ProtocolStatus::playerRecord = 0;
...
void ProtocolStatus::getPlayerRecordAndPlayerAmount()
{
	playerAmount = 0;
	playerRecord = 0;

	std::ostringstream query;
	query << "SELECT `players_online`, `record_online` FROM `stats`;";

	DBResult_ptr result = Database::getInstance()->storeQuery(query.str());
	if (result) {
		playerAmount = result->getNumber<uint32_t>("players_online");
		playerRecord = result->getNumber<uint32_t>("record_online");
	}

	g_scheduler.addEvent(createSchedulerTask(1 * 60 * 1000, ProtocolStatus::getPlayerRecordAndPlayerAmount));
}
...
void ProtocolStatus::sendStatusString()
{
	...
	players.append_attribute("online") = std::to_string(playerAmount).c_str();
	players.append_attribute("max") = std::to_string(g_config.getNumber(ConfigManager::MAX_PLAYERS)).c_str();
	players.append_attribute("peak") = std::to_string(playerRecord).c_str();

	pugi::xml_node monsters = tsqp.append_child("monsters");
	monsters.append_attribute("total") = std::to_string(g_config.getNumber(ConfigManager::MONSTER_COUNT)).c_str();

	pugi::xml_node npcs = tsqp.append_child("npcs");
	npcs.append_attribute("total") = std::to_string(g_config.getNumber(ConfigManager::NPC_COUNT)).c_str();
	...
	map.append_attribute("width") = std::to_string(g_config.getNumber(ConfigManager::MAP_WIDTH)).c_str();
	map.append_attribute("height") = std::to_string(g_config.getNumber(ConfigManager::MAP_HEIGHT)).c_str();
...
void ProtocolStatus::sendInfo(uint16_t requestedInfo, const std::string& characterName)
{
	...
	if (requestedInfo & REQUEST_PLAYERS_INFO) {
		output->addByte(0x20);
		output->add<uint32_t>(playerAmount);
		output->add<uint32_t>(g_config.getNumber(ConfigManager::MAX_PLAYERS));
		output->add<uint32_t>(playerRecord);
	}
	...
	if (requestedInfo & REQUEST_MAP_INFO) {
		...
		output->add<uint16_t>(g_config.getNumber(ConfigManager::MAP_WIDTH));
		output->add<uint16_t>(g_config.getNumber(ConfigManager::MAP_HEIGHT));
	}
